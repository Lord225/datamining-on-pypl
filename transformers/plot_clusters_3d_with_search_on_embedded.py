...
def initialize_data():
    global embeddings, clusters, full_embeddings, clusters_embed, x_min, x_max, y_min, y_max, z_min, z_max
    if 'clusters_embed' not in globals():
        embeddings = pd.read_hdf(EMBEDDINGS, key='embeddings_3', mode='r')
        clusters = pd.read_hdf(CLUSTERS, key='clusters', mode='r')
        full_embeddings = pd.read_hdf(FULL_EMBEDDINGS, key='embeddings', mode='r')
        ...
        full_embeddings = full_embeddings.to_numpy()
        ...
        full_embeddings = torch.tensor(full_embeddings, dtype=torch.float32)
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Slider(id='p', min=0.1, max=20, step=0.1, value=2, marks={i: str(i) for i in range(1, 11)}),
...
@app.callback(
    ...
    [Input('p', 'value')],
    [State('scatter-plot', 'relayoutData'),  # Use State to avoid triggering updates from plot movement
     State('scatter-plot', 'figure')]
)
def search_function(n_clicks, search_input, p, relayout_data, current_figure):
    if n_clicks and search_input:
        search_input = search_input.lower()

        print("serach_input", search_input)
        ...
        embeddings = encode_embeddings([search_input])
        embeddings = embeddings.cpu().numpy()
        ...
        # print(embeddings_latent)
 
        # print(clusters_embed)
